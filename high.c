...
clock_t t0,t1; 
 // Make particles
  ...
  while (path != NULL) {
	...
	t0 = clock();
	//from following to t1 = clock()
	//takes up more than 90% time of HighLocalize
      ...
      if (sample[i].probability > threshold) 
	   {
		...
		sample[i].x = sample[i].x + (TURN_RADIUS * (cos(sample[i].theta + path->T) - cos(sample[i].theta))) + (path->D * cos(moveAngle)) + (path->C * cos(moveAngle + M_PI/2));
		sample[i].y = sample[i].y + (TURN_RADIUS * (sin(sample[i].theta + path->T) - sin(sample[i].theta))) + (path->D * sin(moveAngle)) + (path->C * sin(moveAngle + M_PI/2));
...
void HighSlam(TPath *path, TSenseLog *obs)
{
  ...
  if (h_curGeneration == 0) {
	...
	t1=clock();
	printf("high localize time: %d\n", t1-t0);
	t0=clock();
    HighUpdateAncestry(path, obs);
	t1=clock();
	printf("high HighUpdateAncestry time: %d\n", t1-t0);
