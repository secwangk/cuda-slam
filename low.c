...
#define REC 1
int FILE_FORMAT;
int pBest;//WK, for savedParticle[best]
FILE *readFile;
...
void LowSlam(int &continueSlam, TPath **path, TSenseLog **obs)
{
  ...
  while ((continueSlam) && (counter < LOW_DURATION))
  {
	...
	//We update ancestry and the map if and only if( overflow > 0)
    if (overflow > 0) { 
      overflow--;
...
//WK for testing the possiblity of cuda insertion into slam code, the function MatAdd() worked properly  
//  MatAdd();
}
